...
public:
  ...
  shared_ptr<Node<T>> findParent(shared_ptr<Node<T>> child);
  shared_ptr<Node<T>> findParent(shared_ptr<Node<T>> child, shared_ptr<Node<T>> parent);
...
shared_ptr<Node<T>> BTree<T>::find(const T& item, shared_ptr<Node<T>> node_ptr) {
  if (node_ptr) {
    if (node_ptr->data == item) {
      ...
      shared_ptr<Node<T>> left_result = find(item, node_ptr->left);
...
template <typename T>
shared_ptr<Node<T>> BTree<T>::findRightMostNode(shared_ptr<Node<T>> node_ptr) {
  if (node_ptr) {
    shared_ptr<Node<T>> nextRight = findRightMostNode(node_ptr->right);
    if (nextRight) {
      return nextRight;
    } 
...
template<typename T>
shared_ptr<Node<T>> BTree<T>::findParent(shared_ptr<Node<T>> child) {
  return findParent(child, root);
}

template<typename T>
shared_ptr<Node<T>> BTree<T>::findParent(shared_ptr<Node<T>> child, shared_ptr<Node<T>> node_ptr) {
  if (node_ptr == nullptr || node_ptr == child) {
    return nullptr;
  }

  if ((node_ptr->left != nullptr && node_ptr->left == child) || (node_ptr->right != nullptr && node_ptr->right == child)) {
    return node_ptr; // is the parent
  }

  shared_ptr<Node<T>> left_parent = findParent(child, node_ptr->left);
  if (left_parent != nullptr) {
    return left_parent;
  }

  return findParent(child, node_ptr->right);
}
